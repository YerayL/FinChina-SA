...
def main():
    ...
    if args.data_file is None:
        ...
        with open(args.data_file, 'r') as f:
            FSAData = json.load(f)
            examples = [data["input"].strip() for data in FSAData]  # Modified based on the data format
            # examples = [json.loads(l.strip())["conversations"][0]["value"] for l in f.readlines()]  # Modified based on the data format
    ...
    if args.interactive:
        ...
        results = []
        for index in tqdm(range(len(examples))):
